class LoaderWinners {
    ...
    async getWinners(page, limit, sort, order) {
        try {
            const response = await fetch(`${this.serverPath}${this.winners}${page || limit ? `?${this.paramsPage}=${page}&${this.paramsLimit}=${limit}` : ''}${sort ? `?${this.paramsSort}=${sort}` : ''}${order ? `?${this.paramsOrder}=${order}` : ''}`);
